...
class Event(metaclass=ABCMeta):
    ...
    """
    Base class of events. An event that occurs at a given time (a float value)
    and must be processed by a given server.
    """
    ...
    @abstractmethod
    def process(self):
        """Execute the action associated with the event (must be defined in 
        derived classes)."""
        pass
...
class Server:
    ...
    """
    Servers are responsible for the implementation of the methods that will do
    the work associated with event processing. They can generate new events 
    that are inserted in the calendar.
    """
...
class Calendar:
    ...
    def put(self, event):
        ...
        if event.time() < self.__current_time:
            raise ValueError('New event preceds last removed event')
    ...
    def process_events_until(self, time):
        ...
        specified time (or queue is empty)."""
        while not self.__queue.empty():
            event = self.__queue.get()
            if event.time() <= time:
                self.__current_time = event.time()
