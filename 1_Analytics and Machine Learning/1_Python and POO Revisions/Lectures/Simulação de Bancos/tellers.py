...
class Client:
    ...
    def __init__(self, work):
        "Create a client with the specified amount of work."
    ...
    def work(self):
        "Report amount of work done for this client."
...
class QueueingSystem(Server,metaclass=ABCMeta):
    """
    Base (abstract) class for all client queueing systems.
    """

    def __init__(self, calendar):
        "Create a queueing server associated with the given calendar."
        Server.__init__(self, calendar)
        self.__free_tellers = Queue()

    def new_client(self, client):
        """A new client to attend. Either send it to a free teller (if
        available) or to waiting queue."""
    ...
    def free_teller(self, teller):
        """A new free teller. Send it a client (if available) or put in waiting 
        queue."""
...
class RandomQueue(QueueingSystem):
    ...
    def has_waiting_client(self):
        return len(self.__waiting_clients) > 0
...
class Teller(Server):
    ...
    def attend_client(self, client):
        """Do the work required by the client (takes time). Afterwards, notify
...
def simple_simulation(num_clients, work, arrival_time, num_tellers, rate, 
                      queueing_system, filename):
    calendar = Calendar()
    ...
    with open(filename + '_wait.dat', 'w') as outfile:
        for client in clients:
            ...
            print(client.work(), arrival, departure, file=outfile)

    with open(filename + '_free.dat', 'w') as outfile:
        for teller in tellers:
            freetimes = teller.free_times()
            for t in freetimes:
                print(t, file=outfile, end=' ')
            print(file=outfile)
...
if __name__ == '__main__':
    t = simple_simulation(100, lambda i: 2, lambda i: 3.0 * i, 1, 
                          lambda i: 1, SingleQueue, 
                          'single_oneteller_nooverlap')
    print('Total time for single queue, one teller, no overlap is', t)
    t = simple_simulation(100, lambda i: 2, lambda i: 1.0 * i, 1, 
                          lambda i: 1, SingleQueue, 
                          'single_oneteller_overlap')
    print('Total time for single queue, one teller, overlap is', t)
    t = simple_simulation(100, lambda i: 2, lambda i: 3.0 * i, 1, 
                          lambda i: 1, RandomQueue, 
                          'random_oneteller_nooverlap')
    print('Total time for random queue, one teller, no overlap is', t)
    t = simple_simulation(100, lambda i: 2, lambda i: 1.0 * i, 1, 
                          lambda i: 1, RandomQueue, 
                          'random_oneteller_overlap')
    print('Total time for random queue, one teller, overlap is', t)
    t = simple_simulation(100, lambda i: 2 + random.random(), 
                          lambda i: random.uniform(0, 2500), 2, 
                          lambda i: 1, SingleQueue, 
                          'single_randomclient_twotellers')
    print('Total time for single queue, two tellers, random clients', t)
    t = simple_simulation(100, lambda i: 2 + random.random(), 
                          lambda i: random.uniform(0, 2500), 2, 
                          lambda i: 1, RandomQueue, 
                          'random_randomclient_twotellers')
    print('Total time for random queue, two tellers, random clients', t)
